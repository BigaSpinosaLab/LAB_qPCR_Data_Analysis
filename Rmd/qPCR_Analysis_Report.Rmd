...
packages <- c("pdftools", "stringr", "openxlsx", "dplyr", "ggplot2", "ggpubr", 
              "ggsignif", "readxl", "textshape", "tidyr", "tinytex", 
              "rmarkdown", "ggrepel", "EnvStats", "reshape2", "purrr", "knitr", 
              "kableExtra", "ComplexHeatmap")

# Instalación y carga silenciosa
...
ct_data <- params$ct_data
dd_data <- params$dd_data
pal <- params$pal
...
n_genes <- length(unique(df_mean$Gene))
...
```{r infoCond, echo=FALSE}
  ...
  values <- as.data.frame(ct_data[[1]])
...
```{r PCA_CT, echo=FALSE, warning=FALSE, fig.align = "center"}
  mat1 <- lapply(ct_data, function(x) {
